...
class GAN(object):
    ...
    def discriminator(self, x, is_training=True, reuse=False):
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            out_logit = tf.nn.sigmoid(logits)

            return out_logit, logits, lrelu3

    def generator(self, z, out_channel_dim, is_training=True):
        # Network Architecture is exactly same as in infoGAN (https://arxiv.org/abs/1606.03657)
        # Architecture : FC1024_BR-FC7x7x128_BR-(64)4dc2s_BR-(1)4dc2s_S
        with tf.variable_scope('generator', reuse=False if is_training else True):
            # First fully connected layer
            x_1 = tf.layers.dense(z, 2*2*512)

            # Reshape it to start the convolutional stack
            deconv_2 = tf.reshape(x_1, (-1, 2, 2, 512))
            batch_norm2 = tf.layers.batch_normalization(deconv_2, training=is_training)
            # Leaky Relu
            lrelu2 = lrelu(batch_norm2)
    ...
    def model_loss(input_real, input_z, out_channel_dim):
        ...
        d_loss_real = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=d_logits_real,
                                                    labels=tf.ones_like(d_model_real) * label_smoothing), name="d_loss_real")
        d_loss_fake = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=d_logits_fake,
                                                    labels=tf.zeros_like(d_model_fake)),name="d_loss_fake")

        d_loss = d_loss_real + d_loss_fake

        g_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=d_logits_fake,
                                                    labels=tf.ones_like(d_model_fake) * label_smoothing), name="g_loss")
